"""Widen job_matches' primary key from job_id alone to (job_id, prompt_version), and add the
score_breakdown column used by rubric-based prompts (job_match_v4/v5+).

Why this can't just be `Base.metadata.create_all(engine)`: that call only creates tables that
don't exist yet, it never alters an existing table's columns or constraints (see
shared/job_data.py, shared/job_match_data.py -- both are plain declarative models with no
migration framework backing them, by design, since this repo has no Alembic). Widening the PK is
a real ALTER TABLE.

Guarded by information_schema checks on top of the runner's own schema_migrations tracking table
(see scripts/run_migrations.py) -- defense in depth in case this is ever re-applied outside the
runner (e.g. a tracking row deleted by hand), not just relied on for idempotency.
"""
